...
class BaseX2HAttLayer(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, n_heads, edge_feat_dim, r_feat_dim, act_fn="relu", norm=True, ew_net_type="r", out_fc=True):
        super().__init__()
    ...
    def forward(self, h, r_feat, edge_feat, edge_index, e_w=None):
        ...
        alpha = scatter_softmax((q[dst] * k / np.sqrt(k.shape[-1])).sum(-1), dst, dim=0, dim_size=N)  # [num_edges, n_heads]
...
class BaseH2XAttLayer(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, n_heads, edge_feat_dim, r_feat_dim, act_fn="relu", norm=True, ew_net_type="r"):
        super().__init__()
        self.input_dim = input_dim
        self.hidden_dim = hidden_dim
        self.output_dim = output_dim
...
class MLP_condition(nn.Module):
    ...
    def initialize_as_identity(self):
        identity_matrix = torch.eye(self.layer.out_features, self.layer.in_features)
        identity_matrix[:, self.layer.out_features :] = 0
        with torch.no_grad():
            self.layer.weight.copy_(identity_matrix)
...
class UniTransformerO2TwoUpdateGeneral(nn.Module):
    def __init__(
        ...
        # self condition
        self.h_mlps = nn.ModuleList([MLP_condition(2 * self.hidden_dim, self.hidden_dim) for _ in range(self.num_blocks)])
        for h_mlp in self.h_mlps:
            h_mlp.initialize_as_identity()

        self.x_mlps = nn.ModuleList([MLP_condition(6, 3) for _ in range(self.num_blocks)])
        for x_mlp in self.x_mlps:
            x_mlp.initialize_as_identity()
    ...
    def _connect_edge(self, x, mask_ligand, batch):
        if self.cutoff_mode == "radius":
            ...
            edge_index = batch_hybrid_edge_connection(x, k=self.k, mask_ligand=mask_ligand, batch=batch, add_p_index=True)
        else:
    ...
    def forward(self, h, x, mask_ligand, batch, return_all=False, fix_x=False, h0=None, x0=None):
        all_x = [x]
        all_h = [h]
        ...
        for b_idx in range(self.num_blocks):
            ...
            # self condition
            x = self.x_mlps[b_idx](torch.cat([x, x0], -1))
            h = self.h_mlps[b_idx](torch.cat([h, h0], -1))
