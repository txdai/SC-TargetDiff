...
class ScorePosNet3D(nn.Module):
    def __init__(self, config, protein_atom_feature_dim, ligand_atom_feature_dim):
        ...
        self.posterior_mean_ct_coef = to_torch_const((1.0 - alphas_cumprod_prev) * np.sqrt(alphas) / (1.0 - alphas_cumprod))
    ...
    def forward(
        ...
        if init_ligand_pos0 is None:
            ...
            if self.time_emb_dim > 0:
                if self.time_emb_mode == "simple":
                    input_ligand_feat0 = torch.cat([init_ligand_v0, (time_step / self.num_timesteps)[batch_ligand].unsqueeze(-1)], -1)
                elif self.time_emb_mode == "sin":
                    time_feat = self.time_emb(time_step)
                    input_ligand_feat0 = torch.cat([init_ligand_v0, time_feat], -1)
                else:
                    raise NotImplementedError
            else:
                input_ligand_feat0 = init_ligand_v0

            h_protein = self.protein_atom_emb(protein_v)
            init_ligand_h0 = self.ligand_atom_emb(input_ligand_feat0)

            if self.config.node_indicator:
                h_protein = torch.cat([h_protein, torch.zeros(len(h_protein), 1).to(h_protein)], -1)
                init_ligand_h0 = torch.cat([init_ligand_h0, torch.ones(len(init_ligand_h0), 1).to(h_protein)], -1)

            h_all_0, pos_all_0, _, _ = compose_context(
                h_protein=h_protein,
                h_ligand=init_ligand_h0,
                pos_protein=protein_pos,
                pos_ligand=init_ligand_pos0,
                batch_protein=batch_protein,
                batch_ligand=batch_ligand,
            )

        outputs = self.refine_net(h_all, pos_all, mask_ligand, batch_all, return_all=return_all, fix_x=fix_x, h0=h_all_0, x0=pos_all_0)
        ...
        preds = {"pred_ligand_pos": final_ligand_pos, "pred_ligand_v": final_ligand_v, "final_h": final_h, "final_ligand_h": final_ligand_h}
        if return_all:
            ...
            preds.update({"layer_pred_ligand_pos": final_all_ligand_pos, "layer_pred_ligand_v": final_all_ligand_v})
        return preds
    ...
    def q_v_pred_one_timestep(self, log_vt_1, t, batch):
        ...
        log_probs = log_add_exp(log_vt_1 + log_alpha_t, log_1_min_alpha_t - np.log(self.num_classes))
        return log_probs
    ...
    def q_v_pred(self, log_v0, t, batch):
        ...
        log_probs = log_add_exp(log_v0 + log_cumprod_alpha_t, log_1_min_cumprod_alpha - np.log(self.num_classes))
        return log_probs
    ...
    def _predict_x0_from_eps(self, xt, eps, t, batch):
        pos0_from_e = extract(self.sqrt_recip_alphas_cumprod, t, batch) * xt - extract(self.sqrt_recipm1_alphas_cumprod, t, batch) * eps
        return pos0_from_e

    def q_pos_posterior(self, x0, xt, t, batch):
        # Compute the mean and variance of the diffusion posterior q(x_{t-1} | x_t, x_0)
        pos_model_mean = extract(self.posterior_mean_c0_coef, t, batch) * x0 + extract(self.posterior_mean_ct_coef, t, batch) * xt
    ...
    def kl_pos_prior(self, pos0, batch):
        ...
        kl_prior = normal_kl(torch.zeros_like(pos_model_mean), torch.zeros_like(pos_log_variance), pos_model_mean, pos_log_variance)
        kl_prior = scatter_mean(kl_prior, batch, dim=0)
        return kl_prior
    ...
    def get_diffusion_loss(self, protein_pos, protein_v, batch_protein, ligand_pos, ligand_v, batch_ligand, time_step=None):
        ...
        # 4. self condition with probability
        if torch.rand((1)) < 0.5:
            preds = self(
                protein_pos=protein_pos,
                protein_v=protein_v,
                ...
                batch_ligand=batch_ligand,
                time_step=time_step,
                init_ligand_pos0=preds["pred_ligand_pos"].detach(),
                init_ligand_v0=preds["pred_ligand_v"].detach(),
            )
        ...
        kl_v = self.compute_v_Lt(
            log_v_model_prob=log_v_model_prob, log_v0=log_ligand_v0, log_v_true_prob=log_v_true_prob, t=time_step, batch=batch_ligand
        )
    ...
    @torch.no_grad()
    def likelihood_estimation(self, protein_pos, protein_v, batch_protein, ligand_pos, ligand_v, batch_ligand, time_step):
        protein_pos, ligand_pos, _ = center_pos(protein_pos, ligand_pos, batch_protein, batch_ligand, mode="protein")
        ...
        # t = [T-1, ... , 0]
        kl_pos = self.compute_pos_Lt(pos_model_mean=pos_model_mean, x0=ligand_pos, xt=ligand_pos_perturbed, t=time_step, batch=batch_ligand)
        kl_v = self.compute_v_Lt(
            log_v_model_prob=log_v_model_prob, log_v0=log_ligand_v0, log_v_true_prob=log_v_true_prob, t=time_step, batch=batch_ligand
        )
        return kl_pos, kl_v
    ...
    def sample_diffusion(
        ...
        protein_pos, init_ligand_pos, offset = center_pos(protein_pos, init_ligand_pos, batch_protein, batch_ligand, mode=center_pos_mode)

        pos_traj, v_traj = [], []
        v0_pred_traj, vt_pred_traj = [], []
        ligand_pos, ligand_v = init_ligand_pos, init_ligand_v
        ligand_pos0, ligand_v0 = init_ligand_pos, init_ligand_v
        # time sequence
        ...
        for i in tqdm(time_seq, desc="sampling", total=len(time_seq)):
            ...
            preds = self(
                ...
                batch_ligand=batch_ligand,
                time_step=t,
                init_ligand_pos0=ligand_pos0,
                init_ligand_v0=ligand_v0,
            )

            ligand_pos0, ligand_v0 = preds["pred_ligand_pos"], preds["pred_ligand_v"]
        ...
        ligand_pos = ligand_pos + offset[batch_ligand]
        return {"pos": ligand_pos, "v": ligand_v, "pos_traj": pos_traj, "v_traj": v_traj, "v0_traj": v0_pred_traj, "vt_traj": vt_pred_traj}
