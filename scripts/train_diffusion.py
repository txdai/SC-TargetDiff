...
sys.path.append("/home/txdai/research-2023/TargetDiff/")
...
if __name__ == "__main__":
    ...
    model = ScorePosNet3D(
        config.model, protein_atom_feature_dim=protein_featurizer.feature_dim, ligand_atom_feature_dim=ligand_featurizer.feature_dim
    ).to(args.device)

    # load checkpoint
    model.load_state_dict(torch.load("./pretrained_models/pretrained_diffusion.pt")["model"], strict=False)
        ...
        for it in range(1, config.train.max_iters + 1):
            ...
            if it % config.train.val_freq == 0 or it == config.train.max_iters:
                ...
                if best_loss is None or val_loss < best_loss:
                    ...
                    logger.info(f"[Validate] Val loss is not improved. " f"Best val loss: {best_loss:.6f} at iter {best_iter}")
